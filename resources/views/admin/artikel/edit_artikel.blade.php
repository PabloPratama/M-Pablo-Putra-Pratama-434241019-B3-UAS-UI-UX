...
<style>
    ...
    /* Efek outline ungu pada form saat fokus, kecuali untuk #inputContent */
    input:focus, textarea:not(#inputContent):focus, select:focus { outline: none !important; border-color: #7C3AED !important; box-shadow: 0 0 0 1px #7C3AED !important; }
    #inputContent:focus { outline: none !important; }
...
<div class="pt-4 pb-12 px-4 md:px-6 min-h-screen bg-gray-50">
    <div class="max-w-7xl mx-auto">
        <div class="flex items-center gap-4 mb-8">
            ...
            <h1 class="text-xl md:text-2xl font-bold text-gray-800">Edit Artikel</h1>
        </div>
        ...
        <form id="articleForm" class="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div class="space-y-6">
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <button type="button" id="btnSubmit" onclick="updateAndRedirect()" class="px-6 py-2.5 font-bold text-sm text-white rounded-xl shadow-md">Simpan Perubahan</button>
                </div>
            </div>
        </form>
    </div>
...
<script>
    ...
    let stored =
        JSON.parse(sessionStorage.getItem('my_temp_articles') || '[]');
    ...
    let newData = {
        ...
        image:
            document.getElementById('imagePreview').dataset.src || ''
    };
    ...
    if (editId <= 5) {
        ...
        let editedInitial =
            JSON.parse(
                sessionStorage.getItem('edited_initial_articles')
                || '{}'
            );

        editedInitial[editId] = newData;

        sessionStorage.setItem(
            'edited_initial_articles',
            JSON.stringify(editedInitial)
        );

    } else {

        let index =
            stored.findIndex(a => a.id == editId);
    ...
    window.location.href =
        ...
        let initial = initialArticles.map(article => {
        ...
        let art = all.find(a => a.id == editId);
        ...
        if (art.date) {
            ...
            try {
                ...
                if (!isNaN(d.getTime())) {
                    ...
                    document.getElementById('publishDateInput')
