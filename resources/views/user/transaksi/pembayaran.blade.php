...
<div id="payment-alert" class="fixed top-5 left-1/2 transform -translate-x-1/2 z-50 transition-all duration-500 ease-in-out opacity-0 pointer-events-none translate-y-[-20px]">
    <div class="bg-red-500 text-white px-6 py-3 rounded-lg shadow-lg font-semibold flex items-center gap-3 text-sm sm:text-base">
        ...
        <span>Silakan pilih salah satu metode pembayaran terlebih dahulu!</span>
    </div>
</div>
...
<form id="payment-form" action="" method="GET" class="font-poppins text-white min-h-screen pt-2 pb-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
    ...
    <div class="mb-8">
        <h1 class="font-cormorant text-4xl sm:text-5xl font-bold tracking-wide mb-2">Pembayaran</h1>
        <p class="text-gray-400 text-base sm:text-lg max-w-xl">
            Selesaikan pembayaran untuk mendapatkan akses penuh ke novel pilihanmu.
        </p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div class="lg:col-span-2 bg-[#22123F] rounded-xl p-6 sm:p-8 figma-shadow" style="border: 1px solid rgba(139, 92, 246, 0.30);">
            <h2 class="font-cormorant text-2xl sm:text-3xl font-semibold mb-6">Pilih Metode Pembayaran</h2>
            
            <div class="space-y-4 mb-8">
                <div class="method-card flex items-center justify-between px-6 py-4 rounded-lg figma-shadow">
                    <div class="flex items-center gap-5 w-full">
                        <div class="relative z-10 flex-shrink-0">
                            <input type="radio" id="payment_qris" name="metode_pembayaran" value="qris" class="hidden">
                        ...
                        <div class="flex items-center gap-4">
                            <div class="flex items-center justify-center w-12 h-12 rounded-full bg-white flex-shrink-0 p-2.5">
                                <img src="{{ asset('images/As34.png') }}" alt="QRIS" class="w-full h-full object-contain">
                            </div>
                            <span class="text-base sm:text-lg font-bold tracking-wide">QRIS</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="method-card flex items-center justify-between px-6 py-4 rounded-lg figma-shadow">
                    <div class="flex items-center gap-5 w-full">
                        <div class="relative z-10 flex-shrink-0">
                            <input type="radio" id="payment_va" name="metode_pembayaran" value="virtual_account" class="hidden">
                            <div class="custom-radio">
                                <span class="custom-radio-inner"></span>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-4">
                            <div class="flex items-center justify-center w-12 h-12 rounded-full flex-shrink-0" style="background-color: #4CA5FF;">
                                <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"></path>
                                </svg>
                            </div>
                            <span class="text-base sm:text-lg font-bold tracking-wide">Virtual Account</span>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    // Logika klik murni pada komponen .method-card berbentuk DIV
    document.querySelectorAll('.method-card').forEach(card => {
        card.addEventListener('click', function() {
            ...
            if (radio) {
                if (radio.checked) {
                    // Jika diklik pada card yang AKTIF, uncheck radio tersebut
                    radio.checked = false;
                    this.classList.remove('card-selected');
                } else {
                    // Jika diklik pada card yang MATI, bersihkan card lain dulu (gaya radio klasik)
                ...
                // Memicu trigger event change agar tampilan UI custom CSS singkron
                radio.dispatchEvent(new Event('change', { bubbles: true }));
            }
        });
    });

    // LOGIKA REDIRECT DAN NOTIFIKASI PERINGATAN
    document.getElementById('payment-form').addEventListener('submit', function(event) {
        ...
        if (selectedMethod.value === 'qris') {
            window.location.href = "{{ route('transaksi.qris') }}";
        } else if (selectedMethod.value === 'virtual_account') {
            window.location.href = "{{ route('transaksi.virtual_account') }}";
        }
    });
</script>
@endsection