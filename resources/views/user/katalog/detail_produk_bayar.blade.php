        ...
        <div class="flex flex-col md:flex-row gap-6 items-start flex-1 w-full">
            ...
            <div class="flex-1 flex flex-col justify-between w-full h-auto md:h-72 py-1 gap-4 md:gap-0">
                <div>
                    ...
                    <div class="flex items-center gap-1 mt-3 text-[#FFFFFF]">
                <span class="text-sm font-light">Nathania Putri</span>
                    <svg class="w-4 h-4 fill-current text-[#D17CE3] -mt-[2px]" viewBox="0 0 24 24">
                        <path d="M12 2C6.5 2 2 6.5 2 12s4.5 10 10 10 10-4.5 10-10S17.5 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/>
                    </svg>
                </div>
                    <div class="flex flex-wrap items-center gap-x-2 gap-y-2 text-sm text-gray-300 mt-6 font-light">
                        <div class="flex items-center gap-1">
                            <svg class="w-5 h-5 text-amber-400 fill-current transform -translate-y-[1.3px]" viewBox="0 0 20 20">
                ...
                <div class="w-full bg-[#22123F] border border-[#8B5CF6]/20 rounded-xl px-6 py-7 md:py-9 flex flex-col md:flex-row items-center justify-between gap-5 md:gap-6 custom-shadow mt-4">
                    ...
                    <a href="{{ route('transaksi.keranjang') }}" class="bg-gradient-to-r from-[#D17CE3] to-[#B87BFF] text-white font-semibold text-sm px-8 py-3 rounded-[10px] shadow-lg hover:opacity-90 transition-opacity text-center whitespace-nowrap w-full max-w-[220px]">
                        Beli Sekarang
                    </a>
                </div>
                </div>
            ...
            <div class="scroll-container overflow-y-auto pr-2 space-y-3 transition-all duration-300"
                ...
                @php
                    $bab_asli = [
                        ...
                        9 => ['Maaf yang Tak Sempurna', '6 Feb 2025'],
                    ];
                @endphp

                {{-- Loop Bab 1-9 --}}
                @foreach($bab_asli as $num => $data)
                <div class="flex items-center justify-between text-xs py-2.5 border-b border-[#5C287E]/40 last:border-0 select-none">
                    <div class="flex items-center gap-2.5">
                        {{-- Icon Gembok Abu-Abu --}}
                        <svg class="w-4 h-4 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path>
                        </svg>
                        <span class="font-medium text-white">Bab {{ $num }}</span>
                        <span class="text-gray-400">-</span>
                        <span class="text-gray-400 truncate max-w-[140px] sm:max-w-[180px] md:max-w-[340px]">{{ $data[0] }}</span>
                    </div>
                    <span class="text-gray-500 text-[11px] font-light">{{ $data[1] }}</span>
                </div>
                @endforeach
                ...
                <div x-show="babTerbuka" x-collapse class="space-y-3 pt-1">
                    ...
                    <div class="flex items-center justify-between text-xs py-2.5 border-b border-[#5C287E]/40 last:border-0 select-none">
                        <div class="flex items-center gap-2.5">
                            ...
                            <span class="font-medium text-white">Bab {{ $i }}</span>
                            <span class="text-gray-400">-</span>
                            <span class="text-gray-400 italic">Judul Kisah Sambungan Kelanjutan {{ $i }}</span>
                        </div>
                        <span class="text-gray-500 text-[11px] font-light">{{ $i }} Feb 2025</span>
                    </div>
                    @endfor
                </div>
            </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex gap-4 p-3 rounded-xl cursor-pointer bg-[#18092F] border border-[#8B5CF6]/20 transition-all duration-300 ease-out hover:-translate-y-1.5 hover:border-[#D17CE3] hover-recommend-shadow"
                        ...
                        <div class="flex flex-col justify-center flex-1">
                            ...
                            <div class="flex items-center gap-1 mt-12 text-sm text-amber-400">
                                <svg class="w-4 h-4 text-amber-400 fill-current transform -translate-y-[1px]" viewBox="0 0 20 20">
                                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
    ...
    <div class="bg-[#22123F] rounded-xl p-6 custom-shadow border border-[#8B5CF6]/20 space-y-6 relative pb-16" x-data="{ komentarTerbuka: false }">
        ...
        <div>
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="{{ route('katalog.komentar') }}" class="bg-[#D17CE3] text-white text-sm font-medium px-6 py-2 rounded-lg hover:bg-opacity-90 transition-colors">
                Tulis Komentar
                </a>
            </div>

            <div id="container-komentar-pembaca" class="scroll-container overflow-y-auto pr-2 space-y-5 transition-all duration-300"
                ...
                {{-- Komentar Tambahan --}}
                <div id="komentar-tambahan" x-show="komentarTerbuka" x-collapse class="space-y-5 pt-1">
                    {{-- Komentar 4 --}}
                    <div class="grid grid-cols-1 md:grid-cols-[220px_1fr] gap-2 md:gap-4 items-start text-xs pb-4 border-b border-[#5C287E]/30">
                        <div class="flex items-start gap-3">
                            <div class="w-10 h-10 rounded-full bg-[#18092F] flex items-center justify-center text-gray-400 border border-[#8B5CF6]/30 flex-shrink-0">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function formatWaktu(timestamp) {
        // Jika data lama tidak punya timestamp, kembalikan teks aslinya
        ...
        const minggu = Math.floor(selisih / 604800000);
    ...
    const containerTambahan = document.getElementById('komentar-tambahan');
    
    if (containerTambahan && listKomentar.length > 0) {
        listKomentar.forEach(k => {
            let bintang = '★'.repeat(k.rating) + '☆'.repeat(5 - k.rating);
            let waktuLabel = formatWaktu(k.waktu); 
            
            let newCard = document.createElement('div');
            newCard.className = 'grid grid-cols-1 md:grid-cols-[220px_1fr] gap-2 md:gap-4 items-start text-xs pb-4 border-b border-[#5C287E]/30';
            newCard.innerHTML = `
                <div class="flex items-start gap-3">
                    <div class="w-10 h-10 rounded-full bg-[#18092F] flex items-center justify-center text-gray-400 border border-[#8B5CF6]/30 flex-shrink-0">
                        <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5-4-8-4z"/></svg>
                    ...
                    <div class="flex flex-col space-y-1">
                        <span class="font-semibold text-white text-sm">${k.nama}</span>
                        <div class="flex items-center gap-2">
                            <div class="flex text-amber-400 text-[9px]">${bintang}</div>
                            <span class="text-[10px] text-gray-500 font-light">${waktuLabel}</span>
                        </div>
                    </div>
                </div>
                <div class="pt-1">
                    <p class="text-gray-300 leading-relaxed font-light text-sm">${k.isi}</p>
                </div>
            `;
            containerTambahan.appendChild(newCard);
        });
    }
});
</script>
